...
// 3 y * 12 m * 4 w
const TenantOperationData: Array<TenantOperationRecord> = flatten(
  range(1, 3 * 12 * 4 + 1)
    .map(i => ({
...
const BizTypeOperationData: Array<BizTypeOperationRecord> = flatten(
  range(1, 3 * 12 * 4 + 1)
    .map(i => ({
      year: 2016 + Math.floor(i / (12 * 4)),
      month: Math.ceil((i / 4) % 12),
    ...
    .map((t, i) => {
      return uniq(TenantInfoData.map(i => i[1])).map(bizType => ({
        ...
        month: t.month,
        week: t.week,
        bizType: bizType, // 业态名称
        area: rand(40, 2), // 业态面积
        areaPct: rand(1, 2), // 业态面积 全场占比
        sales: rand(1000, 2), // 业态营业额
        salesPct: rand(1, 2), // 业态营业额 全场占比
        salesLP: rand(1, 2), // 业态营业额 环比 last period
        salesSPLY: rand(1, 2), // 业态营业额 同比 same period last year
        spa: rand(1, 2), // 业态坪效 sales per area
        spaLP: rand(1, 2), // 业态坪效 环比
        spaSPLY: rand(1, 2), // 业态坪效 同比
